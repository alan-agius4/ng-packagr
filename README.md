...
recommended to provide the name of the external dependency. This can be done by using `namedExternals` in the config section like so;
...
Note: Don't forget to include `react` and `react-dom` in your `namedExternals` so that you're not bundling those dependencies!
