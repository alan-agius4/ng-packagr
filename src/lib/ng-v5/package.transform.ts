...
import { concatMap, map, switchMap, takeLast, tap, debounceTime, mapTo, catchError, startWith } from 'rxjs/operators';
        ...
        tap(fileChange =>
          graph.filter(isEntryPoint).forEach((node: EntryPointNode) => {
            ...
            if (fileChange.event === 'unlinkDir') {
              node.fileCache.delete(fileChange.filePath, DeleteStrategy.PartialMatch);
            } else {
              node.fileCache.delete(fileChange.filePath);
            }
          })
        ),
        startWith(undefined),
